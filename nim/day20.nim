...
import std/[deques, math, options, strformat, tables]
...
proc toCharTab(file:string):TabR2i[char] =
  var
...
proc echoCharTab(tab:TabR2i[char]) =
...
proc toMoveTab(ctab:TabR2i[char]):(TabR2i[seq[Vec2i]],Vec2i,Vec2i) =
...
proc walk(ctab:TabR2i[char]):int =
  let
...
proc toMoveTab3i(ctab:TabR2i[char]):(TabR2i[seq[Vec3i]],Vec3i,Vec3i) =
  let
...
proc walk3i(ctab:TabR2i[char]):int =
  let
...
# Compile command: 'nim c --hints=off --warnings=off  -d:danger -d:release --opt:speed -o:out/time_day20 nim/day20.nim'
# Run command: 'time ./out/time_day20'
# Day20
# Part1 454
# Part2 5744
