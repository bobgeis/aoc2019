...
proc toMoveTab3i(ctab:Tab2i[char]):(Tab3i[seq[Vec3i]],Vec3i,Vec3i) =
  let
    (mins,maxs) = ctab.getMinMax
    mids:Vec3i = [maxs.x div 2, maxs.y div 2, 0]
  var
    mtab = newTable[Vec3i,seq[Vec3i]]()
    portals = newTable[set[char],seq[Vec3i]]()
    start,stop: Vec3i
  for y in mins.y..maxs.y:
    for x in mins.x..maxs.x:
      let
      ...
      if ctab.getOrDefault(v2,' ') == '.':
        mtab[v3] = newSeq[Vec3i]()
        for dir in dirs:
          let c = ctab.getOrDefault(v2 + dir, ' ')
          if c == '.':
            mtab[v3].add dir.toVec3(0)
            # debug v, mtab[v]
          elif c in 'A'..'Z':
            let
              c2 = ctab[v2 + dir + dir]
              cset = {c,c2}
            # debug c, c2, cset, typeof(cset)
            if not portals.hasKey(cset): portals[cset] = newSeq[Vec3i]()
            portals[cset].add v3
  for k,vs in portals:
    if k == {'A','A'}:
      start = vs[0]
